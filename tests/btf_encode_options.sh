...
cp "$obj" "$elf_obj"
pahole -J --btf_encode_force "$elf_obj" 2>/dev/null
if [ $? -ne 0 ]; then
...
pahole --btf_encode_detached="$btf_novars" --skip_encoding_btf_vars "$obj" 2>/dev/null
if [ $? -ne 0 ]; then
...
pahole --btf_encode_detached="$btf_noenum64" --skip_encoding_btf_enum64 "$obj" 2>/dev/null
if [ $? -ne 0 ]; then
...
pahole --btf_encode_detached="$btf_repro2" --reproducible_build "$obj" 2>/dev/null
if [ $? -ne 0 ]; then
...
pahole --btf_encode_detached="$btf_all" --btf_gen_all "$obj" 2>/dev/null
if [ $? -ne 0 ]; then
...
pahole --btf_encode_detached="$btf_floats" --btf_gen_floats "$obj" 2>/dev/null
if [ $? -ne 0 ]; then
...
pahole --btf_encode_detached="$btf_nodecl" --skip_encoding_btf_decl_tag "$obj" 2>/dev/null
if [ $? -ne 0 ]; then
...
pahole --btf_encode_detached="$btf_notype" --skip_encoding_btf_type_tag "$obj" 2>/dev/null
if [ $? -ne 0 ]; then
...
pahole --btf_encode_detached="$btf_noatomic" --skip_emitting_atomic_typedefs "$obj" 2>/dev/null
if [ $? -ne 0 ]; then
...
pahole --btf_features_strict=default --btf_encode_detached="$btf_strict" "$obj" 2>/dev/null
if [ $? -ne 0 ]; then
